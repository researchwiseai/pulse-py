...
class StreamlinedWorkflowValidator:
    ...
    def validate_asset_count_limit(self) -> WorkflowValidationResult:
        ...
        try:
            ...
            expected_breakdown = {
                "distributions": 2,
                "attestations": 2,  # Separate .attestation files from GitHub Actions
                "signatures": 4,
                "sbom": 1,
                "provenance": 0,  # Provenance included in attestations
            }
            ...
            for artifact_type, expected_count in expected_breakdown.items():
                ...
                if actual_count != expected_count:
                    validation_errors.append(
                        f"{artifact_type}: expected {expected_count}, "
                        f"got {actual_count}"
                    )
            ...
            if validation_errors:
                return WorkflowValidationResult(
                    ...
                    f"Total assets: {total_assets}, Expected: "
                    f"{sum(expected_breakdown.values())}",
                )
    ...
    def check_github_link_format(self, link: str) -> bool:
        ...
        github_pattern = (
            r"https://github\.com/researchwiseai/pulse-py/blob/main/"
            r"[A-Za-z0-9._/-]+\.md|"
            r"https://github\.com/researchwiseai/pulse-py/blob/main/"
            r"[A-Z][A-Z_-]*"
        )
    ...
    def validate_compliance_document_accessibility(self) -> WorkflowValidationResult:
        ...
        Requirement 2.1: Release notes SHALL contain a "Compliance & Verification"
        section
        ...
        try:
            ...
            if missing_docs:
                return WorkflowValidationResult(
                    ...
                    False,
                    f"Missing or inaccessible compliance documents: "
                    f"{', '.join(missing_docs)}",
                    f"Accessible: {len(accessible_docs)}/"
                    f"{len(self.required_compliance_docs)}",
                )

            return WorkflowValidationResult(
                "Compliance Document Accessibility",
                True,
                f"All {len(accessible_docs)} compliance documents are " f"accessible",
    ...
    def validate_release_notes_compliance_links(self) -> WorkflowValidationResult:
        ...
        Requirement 3.4: Release notes SHALL include a "Compliance & Verification"
        section with direct links to relevant documents
        """
        ...
        try:
            ...
            self.log(
                f"Extracted release notes template "
                f"({len(release_notes_template)} chars)"
            )
            ...
            if issues:
                return WorkflowValidationResult(
                    ...
                    f"Found links: {len(found_links)}/"
                    f"{len(self.required_compliance_links)}",
                )
            ...
            return WorkflowValidationResult(
                ...
                f"Validated {len(found_links)} compliance links and "
    ...
    def validate_workflow_configuration(self) -> WorkflowValidationResult:
        """
        Validate that the GitHub Actions workflow is properly configured for
        streamlined release.
...
"""
        ...
        if passed == total:
            ...
            report += (
                "All validation checks passed. The streamlined workflow is "
            ...
            report += (
                "Significant issues found. Major work needed before workflow "
                "can be used.\n"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        "(default: WORKFLOW_VALIDATION_REPORT.md)",
    )
