...
from pulse_client.core.models import ThemesResponse, SentimentResponse, SimilarityResponse, ExtractionsResponse
...
class DummyDSLClient:
    ...
    def compare_similarity(self, texts, fast=True, flatten=True):
        self.called['compare_similarity'] = dict(texts=list(texts), fast=fast, flatten=flatten)
        size = len(texts)
...
def test_workflow_graph_and_aliasing():
    ...
    graph = wf.graph()
    assert set(graph.keys()) == {'theme_generation', 'theme_extraction', 'sentiment', 'sentiment_2'}
