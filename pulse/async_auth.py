"""Async authentication module for OAuth2 credentials."""
...
class _BaseAsyncOAuth2Auth(httpx.Auth):
    """Base HTTPX Auth class for async OAuth2 bearer token authentication."""
    ...
    def sync_auth_flow(self, request: httpx.Request):
        """Synchronous auth flow - raises error for async auth classes."""
        raise RuntimeError(
            "Cannot use async authentication class with synchronous httpx.Client. "
            "Use AsyncCoreClient or the sync auth classes instead."
        )

    async def async_auth_flow(self, request: httpx.Request):
        """Async auth flow implementation."""
...
class AsyncClientCredentialsAuth(_BaseAsyncOAuth2Auth):
    """Async OAuth2 Client Credentials flow authentication."""
...
class AsyncAuthorizationCodePKCEAuth(_BaseAsyncOAuth2Auth):
    """Async OAuth2 Authorization Code flow with PKCE authentication."""
    ...
    def _get_code(self) -> str:
        """Get authorization code - synchronous as it involves user interaction."""
...
def async_auto_auth():
    """
    Automatically retrieves and returns the default async authentication credentials.

    This function provides async-compatible authentication mechanisms.

    Returns:
        The default async authentication credentials or handler.
    """
    if (
        len(os.environ.get("PULSE_CLIENT_SECRET", "")) > 0
        and len(os.environ.get("PULSE_CLIENT_ID", "")) > 0
    ):
        return AsyncClientCredentialsAuth()
    return AsyncAuthorizationCodePKCEAuth()
